    ...
    async def get_response(
        ...
        if file and file_extension:
            if file_extension.lower() in ["jpg", "png", "jpeg", "webp"]:
                ...
                file_text = await self.file_handler.file_handler(
                    file, f".{file_extension}"
                )
                if (
                    file_text
                    and not file_text.startswith("Error")
                    and not file_text.startswith("Unsupported")
                ):
        ...
        has_yielded_image_loading = False
        ...
        try:
            ...
            async for event in self.graph.astream_events(
                ...
                # (A) Router finished — emit image_loading immediately if image path chosen
                if kind == "on_chain_end" and name == "router":
                    output = event.get("data", {}).get("output", {})
                    if (
                        output.get("workflow") == "conversation_with_image"
                        and not has_yielded_image_loading
                    ):
                        has_yielded_image_loading = True
                        yield {
                            "type": "image_loading",
                            "content": "Generating image...",
                        }

                # (B) Stream text chunks from LLM
                STREAMING_NODES = {"conversation", "conversation_with_image"}
                ...
                # (C) Handle node completions
                    ...
                    if name in ["conversation", "conversation_with_image"]:
                        ...
                        if "generated_image" in output and not has_yielded_image:
                            ...
                            yield {"type": "image", "content": generated_image_url}
                    ...
                    # Handle title generation
                        ...
                        if "title" in output and not has_yielded_title:
                            ...
                            yield {"type": "title", "content": generated_title}
            ...
            if not has_yielded_text:
                ...
                if final_state and final_state.values.get("messages"):
                    ...
                    if isinstance(last_message, AIMessage):
                        ...
                        yield {"type": "text", "content": assistant_response}

                    # Check for image in final state
                    if (
                        final_state.values.get("generated_image")
                        and not has_yielded_image
                    ):
            ...
            if assistant_response:
                if generated_title:
                    await self.db.create_session(
                        userId=userId, session_id=session_id, title=generated_title
                    )
            ...
            yield {"type": "error", "content": f"Error in graph execution: {str(e)}"}

    async def get_chat_history(self, session_id: str, user_id: str):
        """Retrieve chat history for a session"""
        return await self.db.get_messages(session_id, user_id)
